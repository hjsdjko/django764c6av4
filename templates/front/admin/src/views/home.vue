...
<div class="content" :style='{"minHeight":"100vh","width":"100%","padding":"30px","background":"url(http://codegen.caihongy.cn/20240320/095a0a9d9da74a8fa6c854b2144c65aa.png) no-repeat center top / cover","height":"auto"}'>
	...
	<div :style='{"width":"100%","margin":"0 0 20px 0","alignItems":"center","flexWrap":"wrap","justifyContent":"center","display":"flex"}'>
		<div :style='{"border":" 1px solid rgba(167, 180, 201,.3)    ","margin":"0 10px","borderRadius":"4px","background":"#fff","display":"flex"}' v-if="isAuth('dianyingshuju','首页总数')">
			...
			<div :style='{"width":"120px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex","order":"1"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","fontWeight":"bold","height":"24px"}'>{{dianyingshujuCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"14px","color":" #a3b1c9","height":"24px"}'>电影数据总数</div>
			</div>
		</div>
		<div :style='{"border":"1px solid rgba(167, 180, 201,.3)  ","margin":"0 10px","borderRadius":"4px","background":"#fff","display":"flex"}' v-if="isAuth('dianyingshujuer','首页总数')">
			<div :style='{"alignItems":"center","background":"#00e682","display":"flex","width":"80px","justifyContent":"center","height":"80px","order":"2"}'>
				<span class="icon iconfont icon-xihuan" :style='{"color":"#fff","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"120px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#374254","fontWeight":"bold","height":"24px"}'>{{dianyingshujuerCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"14px","color":" #a3b1c9","height":"24px"}'>电影数据2总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<!-- echarts -->
	<!-- 5 -->
	<div class="type5" :style='{"alignContent":"flex-start","padding":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="dianyingshujuChart1" class="echarts1" v-if="isAuth('dianyingshuju','首页统计')"></div>
		<div id="dianyingshujuChart2" class="echarts2" v-if="isAuth('dianyingshuju','首页统计')"></div>
		<div id="dianyingshujuChart3" class="echarts3" v-if="isAuth('dianyingshuju','首页统计')"></div>
		<div id="dianyingshujuChart4" class="echarts4" v-if="isAuth('dianyingshuju','首页统计')"></div>
		<div id="dianyingshujuChart5" class="echarts5" v-if="isAuth('dianyingshuju','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	data() {
		return {
			...
			boardBase: {"funnelNum":8,"lineNum":12,"gaugeNum":8,"barNum":16,"pieNum":12},
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            ...
            if (data && data.code === 0) {
				...
				// 动画效果
				this.myChartInterval(1, option.xAxis.data, option.series[0].data, dianyingshujuChart3)
                  //根据窗口的大小变动图表
                window.onresize = function() {
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            ...
            if (data && data.code === 0) {
				...
				// 动画效果
				this.myChartInterval(2, option.yAxis.data, option.series[0].data, dianyingshujuChart4)
